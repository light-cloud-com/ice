...
//
// Pattern C: CI ships signed-but-not-notarized DMGs so the build job never
// blocks on Apple's notary queue. This script is the maintainer-side companion:
// download each macOS DMG from the release, submit to Apple, wait for the
// verdict, staple, and re-upload — all WHILE THE RELEASE IS STILL A DRAFT.
//
// Usage:
//   node scripts/notarize-release.mjs              # uses package.json version
//   node scripts/notarize-release.mjs v0.1.901     # explicit tag
//
// Prerequisites:
//   - Keychain profile "ice-notary" stored via:
//       xcrun notarytool store-credentials "ice-notary" \
//         --apple-id "<your-apple-id>" --team-id "8Y2UTZ2NBZ"
//   - `gh` authenticated with write access to the repo
//   - Run on macOS (needs xcrun notarytool / stapler).
//
...
const KEYCHAIN_PROFILE = "ice-notary";
...
async function main() {
  ...
  const workDir = mkdtempSync(join(tmpdir(), "ice-notarize-"));
