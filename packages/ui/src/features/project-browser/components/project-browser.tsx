      ...
      >
        ...
        <span className="flex-1 ml-1.5 min-w-0">
          {isRenaming ? (
            ...
            <span className="text-ice-md text-ice-text-2 truncate block leading-tight">
              {node.name}
            </span>
          )}
        </span>
        ...
        {!isFolder && node.cards.length > 0 && (
          <span className="text-ice-xs text-ice-text-3 tabular-nums mr-1">
            {node.cards.length}
          </span>
        )}
...
export function ProjectBrowser() {
  ...
  const handleCreate = useCallback(async (type: 'folder' | 'project', parentId?: string) => {
    if (!selectedOrg) return;
  ...
  const buildPath = useCallback((node: ProjectNode, allItems: ProjectNode[]): string => {
    const parts: string[] = [];
    let current: ProjectNode | undefined = node;

    // Walk up via parent_id to build full path
    while (current) {
      const slug = current.slug || current.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
      parts.unshift(slug);
  ...
  const handleNavigateSubpage = useCallback((node: ProjectNode, subpage: string) => {
    const allFlat = flatFolders.concat(items.flatMap(function flatten(n: ProjectNode): ProjectNode[] {
  ...
  const handleOpen = useCallback((node: ProjectNode) => {
    // Build the URL path and navigate
    const path = buildPath(node, flatFolders.concat(items.flatMap(function flatten(n: ProjectNode): ProjectNode[] {
      return [n, ...(n.children || []).flatMap(flatten)];
    })));
    navigate(path);
  }, [buildPath, flatFolders, items, navigate]);
