...
10. **`FORCE_NEW_PROPERTIES` table is partially dead** in
    ...
    every `_` to `.` before lookup, but several map keys _contain_
...
11. **`azure-importer/type-mapper.ts:40` dead key**.
    `'microsoft.web/staticSites'` (capital S) is in TYPE*MAP, but
...
true` if ALL errors are ACCESS*DENIED — may mask real
    permission misconfigurations. \_Origin*:
