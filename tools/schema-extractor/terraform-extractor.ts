...
export class TerraformExtractor {
  ...
  async discover_providers(): Promise<ProviderMetadata[]> {
    ...
    try {
      ...
      for (const item of response.providers) {
        ...
        const data_source_count = docs.filter(
          (d) => d.attributes.category === 'data-sources'
        ).length;
    ...
    try {
      ...
      if (!schema) {
        return this.create_error_result(
          provider_id,
          start_time,
          `Failed to fetch schema for ${provider_id}`
        );
      }
      ...
      if (provider_content?.resource_schemas) {
        for (const [resource_type, resource_schema] of Object.entries(
          provider_content.resource_schemas
        )) {
          try {
      ...
      if (provider_content?.data_source_schemas) {
        for (const [data_source_type, data_source_schema] of Object.entries(
          provider_content.data_source_schemas
        )) {
          try {
  ...
  private convert_attribute(name: string, attr: TerraformAttribute): PropertyDefinition {
    ...
    if (attr.nested_type) {
      ...
      nested_properties = Object.entries(attr.nested_type.attributes).map(([n, a]) =>
        this.convert_attribute(n, a)
      );
    } else {
   ...
   */
  private async fetch_provider_versions(
    namespace: string,
    name: string
   ...
   */
  private async fetch_provider_docs(
    namespace: string,
    name: string,
    version: string
  ): Promise<TerraformDocItem[]> {
    try {
   ...
   * Get provider metadata.
   */
  private async get_provider_metadata(
    namespace: string,
    name: string
