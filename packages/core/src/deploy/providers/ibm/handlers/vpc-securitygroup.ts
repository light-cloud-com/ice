/**
 * IBM VPC Security Group handler — `ibm.vpc.securitygroup`.
 */
...
const TYPE = 'ibm.vpc.securitygroup';
const SDK = 'ibm-vpc';

export const vpc_securitygroup_handler: IBMResourceHandler = {
  async create(name, properties, ctx) {
    const start = Date.now();
    const vpc = await resolveClient(ctx, 'vpc');
    if (!vpc) return sdkMissing(name, TYPE, 'create', start, 'IBM VPC', SDK);
    if (!properties.vpc_id) return err(name, TYPE, 'create', start, 'SG requires properties.vpc_id');
    try {
      const result = await vpc.createSecurityGroup({
      ...
      if (!id) return err(name, TYPE, 'create', start, 'createSecurityGroup returned no id');
  ...
  async delete(name, provider_id, ctx) {
    ...
    try {
      await vpc.deleteSecurityGroup({ id: provider_id });
