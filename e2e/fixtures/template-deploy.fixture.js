...
import { getActionLog, getApiCalls, clearActionLog } from '../utils/action-log-reader';
import { verifyGCPResource } from '../utils/gcp-verify';
...
export class TemplateDeployHelper {
     ...
     */
    async waitForCanvasNodes(expectedMinNodes = 1) {
        try {
            await this.page.waitForFunction((min) => document.querySelectorAll('[data-node-id]').length >= min, expectedMinNodes, { timeout: 20000 });
        }
        catch {
     ...
     */
    async connectGCPViaUI(saKeyJson) {
     ...
     */
    async connectGitHubViaUI(patToken) {
    ...
    async closeAiChat() {
        ...
        if (!(await aiPanel.isVisible({ timeout: 1000 }).catch(() => false)))
            return;
    ...
    async configureDeploy(project, region) {
    ...
    async plan() {
        ...
        try {
            await this.page.waitForFunction(() => {
                ...
                return log.some((e) => e.target.includes('/canvas/deploy/plan') && (e.action === 'api_response' || e.action === 'api_error'));
            }, {}, { timeout: 60000 });
        }
        catch {
        ...
        if (!response)
            return { success: false, error: 'No plan response in action log' };
    ...
    async apply(timeout = 600_000) {
        ...
        try {
            await this.page.waitForFunction(() => {
                const log = window.__ICE_ACTION_LOG__ || [];
                return log.some((e) => e.target.includes('/canvas/deploy/apply') && (e.action === 'api_response' || e.action === 'api_error'));
            }, {}, { timeout });
        }
        catch {
        ...
        if (!response)
            return { success: false, logs, error: 'No deploy response in action log' };
        const status = response.detail.status;
        return {
            success: status === 200,
    ...
    async destroy(timeout = 300_000) {
        ...
        try {
            await this.page.waitForFunction(() => {
                const log = window.__ICE_ACTION_LOG__ || [];
                return log.some((e) => e.target.includes('/canvas/deploy/destroy') && (e.action === 'api_response' || e.action === 'api_error'));
            }, {}, { timeout });
        }
        catch {
        ...
        if (!response)
            return { success: false, error: 'No destroy response in action log' };
        const status = response.detail.status;
        return {
            success: status === 200,
            error: status !== 200 ? JSON.stringify(response.detail.data) : undefined,
    ...
    // ── Screenshots ────────────────────────────────────────────
    async screenshot(name, dir = 'test-results/gcp') {
        const { mkdirSync } = await import('fs');
        mkdirSync(dir, { recursive: true });
    ...
    // ── Action Log ─────────────────────────────────────────────
    async captureAndClearActionLog() {
     ...
     */
    async fetchNodeOverlay(environment = 'development') {
        return this.page.evaluate(async (env) => {
            try {
                ...
                const cardId = parsed?.activeCardId || null;
                if (!cardId)
                    return null;
                const headers = {};
    ...
    verifyResources(resources, project, region) {
        return resources.map((r) => {
            const result = verifyGCPResource(project, region, r);
            return {
                name: r.name,
                type: r.type,
