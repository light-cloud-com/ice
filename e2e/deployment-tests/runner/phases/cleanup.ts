...
export async function runCleanup(
  ...
  const shouldDestroy =
    (overallStatus === 'pass' && destroyOnSuccess) || (overallStatus === 'fail' && destroyOnFailure);

  if (!shouldDestroy) {
    logger.note(
      `Preserving resources (overall=${overallStatus}, destroyOnSuccess=${destroyOnSuccess}, destroyOnFailure=${destroyOnFailure})`,
    );
  ...
  try {
    ...
    for (const ev of log) {
      ...
      if (action === 'api_call') logger.emit({ kind: 'api_call', target, detail, appSeq: seq });
        ...
        logger.emit({ kind: 'api_error', target, status: Number(detail.status ?? 0) || undefined, detail, appSeq: seq });
    }
...
function renderPreservedNotice(scenarioId: string, project: string, region: string): string {
  return [
