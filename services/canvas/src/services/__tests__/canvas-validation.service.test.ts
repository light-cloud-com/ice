/**
 * Unit tests for `services/canvas/src/services/canvas-validation.service.ts`.
 *
 ...
 *
 * The SUT calls `core.validateCanvas` from `@ice/core` via dynamic import. We
 * mock the package so each test can shape the issue list per scenario and so
 * the import-failure fallback branch is reachable.
 */
...
describe('validateCanvas', () => {
  it('returns valid=true with empty errors and a node-count summary when core returns no issues', async () => {
  ...
  it('passes mapped node + edge shapes (id/type/data/parentId) to core.validateCanvas', async () => {
    const nodes = [
  ...
  it('returns the safe fallback (valid=true, engine-unavailable summary) when core import throws', async () => {
