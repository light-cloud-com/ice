/**
 * Tests for `SvgWorkerNode` — thin delegate to SvgCompactNode.
 */
...
describe('SvgWorkerNode', () => {
  it('renders SvgCompactNode', () => {
    const tree = SvgWorkerNode({ node: makeNode(), isSelected: false }) as React.ReactElement;
    expect(tree.type).toBe(mocks.SvgCompactNode);
  });

  it('forwards node and isSelected', () => {
    const node = makeNode();
    const tree = SvgWorkerNode({ node, isSelected: true }) as React.ReactElement;
    expect((tree.props as { node: CanvasNode }).node).toBe(node);
  });

  it('carries displayName "SvgWorkerNode"', () => {
