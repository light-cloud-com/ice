...
test.describe('Phase C: event replay wiring', () => {
    test('card mount triggers /stream replay request', async ({ authenticatedPage }) => {
        ...
        authenticatedPage.on('request', (req) => {
            const url = req.url();
            if (url.includes('/canvas/deploy/stream/'))
                streamRequests.push(url);
            if (url.includes('/canvas/deploy/current/'))
                snapshotRequests.push(url);
            if (url.includes('/canvas/deploy/node-outputs/'))
        ...
        if (streamRequests.length === 0 && snapshotRequests.length === 0 && overlayRequests.length === 0) {
            throw new Error('Expected at least one of /stream, /current, /node-outputs to fire on card mount — ' +
    ...
    test('stream request uses seq cursor on subsequent fetches within same card', async ({ authenticatedPage }) => {
        ...
        const replayCrashes = consoleErrors.filter((e) => e.includes('applyDeployEvent') ||
