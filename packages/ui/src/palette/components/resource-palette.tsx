...
import { CLOUD_PROVIDERS } from '@ice-engine/core/resources';
...
const CATEGORY_DEFS: CategoryDef[] = [
  {
    ...
    color: '#22c55e',
    tooltip:
      'Application servers, background workers, serverless functions, and PaaS platforms that run your code.',
  },
  {
    id: 'Scheduler',
    label: 'Scheduler',
  ...
  },
  {
    ...
    color: '#f59e0b',
    tooltip:
      'Managed databases — relational, document, key-value, and wide-column stores across all providers.',
  },
  {
    id: 'Cache',
    label: 'Cache',
  ...
  },
  {
    ...
    color: '#64748b',
    tooltip:
      'Object storage for files, images, uploads, backups, and static assets. S3, GCS, Blob, OSS, OCI, Spaces.',
  },
  {
    id: 'Security',
    label: 'Security',
...
export const ResourcePalette: React.FC = () => {
  ...
  useEffect(() => {
    if (resolved.type === 'project' && resolved.id) {
      axiosInstance.post('/canvas/projects/get', { projectId: resolved.id })
    ...
    () =>
      COMPONENTS.filter((c) => {
        ...
        const matchesProvider =
          selectedProvider === 'all' || c.providers.includes(selectedProvider as Provider);
      ...
      >
        ...
        <div className="h-full flex flex-col">
          ...
          <div className={cn('shrink-0', showProjects && 'max-h-[40%]')}>
            ...
            >
              <ChevronRight className={cn('w-3 h-3 text-ice-text-3 transition-transform duration-150 shrink-0', showProjects && 'rotate-90')} />
          ...
          >
            <ChevronRight className={cn('w-3 h-3 text-ice-text-3 transition-transform duration-150 shrink-0', showBlocks && 'rotate-90')} />
          ...
          {showBlocks && (
            <div className="px-2 pb-1 shrink-0">
              <div className="relative mb-1">
                ...
                />
                {localSearch && (
                  <button onClick={clearSearch} className="absolute right-1.5 top-1/2 -translate-y-1/2 p-0.5 rounded hover:bg-ice-active">
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        isLocked ? 'text-ice-text-3 opacity-30 cursor-not-allowed' :
                    ...
                    <div key={category.id} className="mb-px">
                      ...
                      {!isSearching && (
                        ...
                        >
                          ...
                          <span className="text-ice-xs text-ice-text-3 ml-auto tabular-nums">
                            {items.length}
                          </span>
                        </button>
                      )}
...
const ComponentItem: React.FC<ComponentItemProps> = ({
  ...
  const [selectedRuntime, setSelectedRuntime] = useState<string | null>(
    component.runtimes?.[0]?.value ?? null
  );
  ...
  return (
    <div
      ...
      style={{ animationDelay: staggerIndex > 0 ? `${staggerIndex * 15}ms` : '0ms' }}
    >
      <Tooltip>
        ...
        >
          ...
          <div className="flex gap-1 mt-1.5">
            {component.providers.map((p) => (
              <span key={p} className="text-ice-2xs font-medium text-ice-text-3 uppercase">{p}</span>
            ))}
          </div>
        </TooltipContent>
      </Tooltip>
