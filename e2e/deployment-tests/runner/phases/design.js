...
export async function runDesign(ctx) {
 ...
 */
async function applyTemplateOverrides(ctx) {
    ...
    for (const [iceType, properties] of entries) {
        ...
        for (const node of matches) {
            ...
            await page.waitForTimeout(300);
            await props.applyProperties(properties);
        }
    }
 ...
 */
async function checkCanvasValidation(ctx) {
    ...
    logger.note(`Canvas validation: ${state.valid ? 'valid' : 'invalid'}, deployable=${state.deployable} ` +
    ...
    const allowedCodes = Array.isArray(allow) ? new Set(allow) : new Set();
    const blockingErrors = issues.filter((i) => i.severity === 'error');
    const blockingWarnings = issues.filter((i) => i.severity === 'warning' && !allowAllWarnings && !allowedCodes.has(i.code));
    if (blockingErrors.length > 0 || blockingWarnings.length > 0 || state.deployable === false) {
        const reasons = [];
        if (blockingErrors.length > 0)
            reasons.push(`${blockingErrors.length} error(s)`);
        if (blockingWarnings.length > 0)
            reasons.push(`${blockingWarnings.length} warning(s)`);
        if (state.deployable === false)
            reasons.push('not deployable');
        return {
            status: 'fail',
 ...
 */
async function drainAndClassify(ctx, step) {
    const { page, logger } = ctx;
