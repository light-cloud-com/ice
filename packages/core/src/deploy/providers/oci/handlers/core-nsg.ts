/**
 * OCI Network Security Group handler — `oci.core.networksecuritygroup`.
 */
...
const TYPE = 'oci.core.networksecuritygroup';
const SDK = 'oci-core';

export const core_nsg_handler: OCIResourceHandler = {
  async create(name, properties, ctx) {
    const start = Date.now();
    const vn = await resolveClient(ctx, 'vnclient');
    if (!vn) return sdkMissing(name, TYPE, 'create', start, 'OCI VirtualNetwork', SDK);
    if (!properties.vcn_id) return err(name, TYPE, 'create', start, 'NSG requires properties.vcn_id');
    try {
      const result = await vn.createNetworkSecurityGroup({
      ...
      const id = result?.networkSecurityGroup?.id as string | undefined;
      if (!id) return err(name, TYPE, 'create', start, 'createNetworkSecurityGroup returned no id');
  ...
  async update(name, provider_id, _properties, _current, ctx) {
    ...
    try {
      await vn.updateNetworkSecurityGroup({
        networkSecurityGroupId: provider_id,
        updateNetworkSecurityGroupDetails: { displayName: name },
      });
  ...
  async delete(name, provider_id, ctx) {
    ...
    try {
      await vn.deleteNetworkSecurityGroup({ networkSecurityGroupId: provider_id });
