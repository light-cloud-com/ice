...
test.describe('Pre-test cleanup', () => {
    ...
    test('destroy + delete every stale project', async ({ page, templateDeploy }) => {
        ...
        console.log(`[cleanup-stale] done — destroyed=${destroyed} skipped=${destroySkipped} deleted=${deleted} delete-failed=${deleteFailed}`);
...
async function destroyViaUI(page, templateDeploy, proj) {
    ...
    const row = page.locator('button:has(button[aria-label="More options"])', { hasText: proj.name }).first();
    ...
    try {
        await templateDeploy.configureDeploy(process.env.ICE_TEST_GCP_PROJECT || '', process.env.ICE_TEST_GCP_REGION || 'us-central1');
    }
    catch {
    ...
    try {
        if (!(await everythingCheckbox.isChecked()))
            await everythingCheckbox.check({ force: true });
    }
    catch {
