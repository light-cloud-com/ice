...
const SCHEMA_SQL = fs.readFileSync(
  path.join(__dirname, '../packages/core/src/schemas/db/schema.sql'),
  'utf-8'
);
        ...
        for (const pattern of RELATIONSHIP_PATTERNS) {
          if (this.matches_pattern(prop_name, pattern.property_suffix)) {
            ...
            for (const target of matching_targets) {
              ...
              const cardinality =
                prop_name.endsWith('s') || prop_name.includes('ids') ? 'many' : 'one';
...
const isMainModule =
  ...
  process.argv[1]?.endsWith('build-schema-db.ts');
