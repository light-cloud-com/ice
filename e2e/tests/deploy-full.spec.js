...
import { getApiCalls, dumpActionLog } from '../utils/action-log-reader';
    ...
    test('plan → apply → verify → destroy → verify removal', async ({ authenticatedPage: page }) => {
        ...
        await reporter.step(page, 'Click Apply', 'click', async () => {
            ...
            if (await applyBtn.isVisible()) {
                ...
                await page.waitForFunction(() => {
                    const log = window.__ICE_ACTION_LOG__ || [];
                    return log.some((e) => e.target.includes('/canvas/deploy/apply') &&
        ...
        if (deployResponse && deployResponse.detail.status === 200) {
            ...
            await reporter.step(page, 'Click Destroy', 'click', async () => {
                ...
                if (await destroyBtn.isVisible()) {
                    ...
                    await page.waitForFunction(() => {
                        const log = window.__ICE_ACTION_LOG__ || [];
                        return log.some((e) => e.target.includes('/canvas/deploy/destroy') &&
                            (e.action === 'api_response' || e.action === 'api_error'));
                    }, {}, { timeout: 120000 });
                }
