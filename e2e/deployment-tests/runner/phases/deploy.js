 ...
 */
function extractRequirements(events) {
    ...
    if (!reqEvent)
        return [];
    const detail = reqEvent.detail || {};
    const data = detail.data || {};
    const arr = Array.isArray(data.requirements) ? data.requirements : [];
    const out = [];
    for (const r of arr) {
        const result = r.result || {};
        const status = String(result.status ?? 'unknown');
        if (status === 'met' || status === 'verified')
...
async function drainActionLog(ctx) {
...
export async function runDeploy(ctx) {
            ...
            if (planResult.success) {
                ...
                if (requirements.length > 0) {
                    ...
                    if (blockers.length > 0) {
                        ...
                        return {
                            status: 'fail',
                            error: `Deploy blocked by ${blockers.length} requirement(s): ` +
...
async function tryRecover(ctx, classified) {
...
function isAllowed(scenario, category) {
    const allow = scenario.recipes.allow;
    const forbid = scenario.recipes.forbid;
    if (forbid.includes('*')) {
        return allow.includes(category);
    }
    if (forbid.includes(category))
        return false;
    return true;
}
function extractSkipReasons(plan) {
    if (!plan || typeof plan !== 'object')
        return [];
    const top = plan;
    const inner = top.plan && typeof top.plan === 'object' ? top.plan : top;
    const skipped = Array.isArray(inner.skipped) ? inner.skipped : [];
...
function summarizePlan(plan) {
    ...
    const top = plan;
    const inner = top.plan && typeof top.plan === 'object' ? top.plan : top;
    const arr = (k) => (Array.isArray(inner[k]) ? inner[k].length : 0);
    const creates = arr('creates');
    const updates = arr('updates');
    const deletes = arr('deletes');
    const skipped = arr('skipped');
    const deployable = typeof inner.deployable_count === 'number' ? inner.deployable_count : creates + updates + deletes;
    return (`creates=${creates} updates=${updates} deletes=${deletes}` +
        (skipped > 0 ? ` skipped=${skipped}` : '') +
        ` deployable=${deployable}`);
}
function startLogTail(ctx) {
    const { page, logger } = ctx;
    let emitted = 0;
