...
export class TestRunner {
    ...
    async start(config) {
        ...
        try {
            this.process = spawn('pnpm', ['exec', 'playwright', 'test', '--config', 'e2e/playwright.config.ts', '--project=gcp-integration'], {
                env,
                cwd: process.cwd(),
        ...
        this.process.stderr?.on('data', (data) => {
            ...
            for (const line of lines) {
                ...
                if (line.includes('npm warn') || line.includes('[WebServer]'))
                    continue;
                this.log(line);
            }
        });
    ...
    writeFallbackReport(exitCode) {
        ...
        const errorLines = this._output.filter((l) => l.includes('Error:') || l.includes('error') || l.includes('FAIL') || l.includes('✘'));
...
<div class="log">${outputHtml}</div>
    ...
    log(msg) {
        ...
        // Keep last 500 lines
        if (this._output.length > 500)
            this._output = this._output.slice(-500);
