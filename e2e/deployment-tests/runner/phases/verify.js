...
export async function runVerify(ctx) {
    ...
    if (deployed.length > 0) {
        ...
        const keys = applyResult?.result && typeof applyResult.result === 'object'
            ? Object.keys(applyResult.result).join(', ')
            : '(none)';
    ...
    if (scenario.expect.resources.length === 0) {
        ...
        if (failed.length > 0) {
            const detail = failed.map((d) => `${d.type} ${d.name}${d.error ? ` — ${d.error.slice(0, 200)}` : ''}`).join('; ');
            return {
                status: 'fail',
    ...
    for (const expected of scenario.expect.resources) {
        ...
        const result = verifyGCPResource(project, region, {
            name: targetName,
            type: expected.kind,
        });
        logger.emit({
...
function extractDeployedResources(applyData) {
    ...
    const arr = Array.isArray(inner.resources) ? inner.resources : [];
    const out = [];
    for (const c of arr) {
...
function findMatch(deployed, expected) {
