...
export class TemplateTestReporter {
    ...
    buildReport() {
        ...
        for (const r of this.records) {
            if (r.overallSuccess && r.errors.length === 0)
                passed++;
            else if (!r.phases.plan.success)
                skipped++;
            else if (r.overallSuccess)
                partial++;
            else
                failed++;
...
<section class="summary-grid">
  ...
  <div class="stat-card fail">${s.resourcesFailed}<small>Failed</small></div>
    ...
    renderErrorSummary(report) {
        const entries = Object.entries(report.errorSummary).filter(([, v]) => v > 0);
        if (entries.length === 0)
            return '<section><h2>Errors</h2><p class="dim">No errors</p></section>';
...
<section class="detail" id="detail-${esc(t.templateId)}" style="display:none">
  ...
  ${logs}
    ...
    renderPhaseTimeline(t) {
        ...
        const dots = phases
            .map((p) => {
            const phase = t.phases[p.key];
            if (!phase || !phase.duration_ms)
                return `<span class="phase-dot pending" title="${p.name}">&#9675;</span>`;
  ...
  <table class="resource-table">
    ...
    getStatus(t) {
        if (!t.phases.plan.success)
            return { label: 'SKIP', class: 'skip' };
        if (t.overallSuccess && t.errors.length === 0)
            return { label: 'PASS', class: 'pass' };
        if (t.overallSuccess)
