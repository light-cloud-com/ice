...
import { z } from 'zod';
import type { ErrorCategory } from '../../utils/error-classifier';

const errorCategoryEnum: [ErrorCategory, ...ErrorCategory[]] = [
  'auth',
  'permission',
  'quota',
  'api_not_enabled',
...
export const scenarioSchema = z.object({
  id: z.string().min(1).regex(/^[a-z0-9][a-z0-9-]*$/i, 'id must be kebab-case'),
  name: z.string().min(1),
...
function interpolate(value: unknown, scenarioId?: string): unknown {
  if (typeof value === 'string') {
    return value.replace(ENV_VAR_RE, (full, name) => {
      if (name === 'SCENARIO_ID' && scenarioId) return scenarioId;
  ...
  if (Array.isArray(value)) return value.map((v) => interpolate(v, scenarioId));
...
export function loadScenario(path: string): LoadResult {
  const source = readFileSync(path, 'utf-8');
  const raw = parseYaml(source);
  if (!raw || typeof raw !== 'object') {
    throw new Error(`Invalid scenario YAML at ${path}: empty or not an object`);
  }
  const scenarioId = (raw as { id?: string }).id;
...
function validateConnections(s: Scenario): void {
  const blockIds = new Set(s.blocks.map((b) => b.id));
  for (const c of s.connections) {
    if (!blockIds.has(c.from)) throw new Error(`connection.from "${c.from}" doesn't match any block.id`);
    if (!blockIds.has(c.to)) throw new Error(`connection.to "${c.to}" doesn't match any block.id`);
  }
