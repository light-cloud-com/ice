...
export const EnvironmentTabBar: React.FC<EnvironmentTabBarProps> = ({ projectId, basePath }) => {
  ...
  // Fetch latest deploy status per environment card
  useEffect(() => {
      ...
      for (const env of environments) {
        try {
          ...
          if (deploys.length > 0) {
            ...
            statuses[env.id] = { status: latest.status, url: latest.deployed_url };
          }
        } catch { /* ignore */ }
      }
  ...
  return (
    <>
      <div className="h-8 flex items-center gap-0.5 px-3 border-b border-ice-border bg-ice-base shrink-0">
