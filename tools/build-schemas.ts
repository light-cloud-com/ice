...
async function extract_terraform_cli(
...
async function extract_terraform(
  config: ExtractorConfig,
  options: BuildOptions
): Promise<ExtractionResult[]> {
...
function print_statistics(stats: {
  ...
  console.log('╠════════════════════════════════════════════════════════════╣');
  console.log(
    `║  Total Resources:           ${String(stats.total_resources).padStart(6)}                       ║`
  );
  console.log(
    `║  Pulumi Resources:          ${String(stats.pulumi_resources).padStart(6)}                       ║`
  );
  console.log(
    `║  Terraform Resources:       ${String(stats.terraform_resources).padStart(6)}                       ║`
  );
  ...
  console.log(
    `║  Pulumi Providers:          ${String(stats.pulumi_providers).padStart(6)}                       ║`
  );
  console.log(
    `║  Terraform Providers:       ${String(stats.terraform_providers).padStart(6)}                       ║`
  );
  console.log(
    `║  Categories:                ${String(stats.categories.length).padStart(6)}                       ║`
  );
  console.log(
...
async function generate_output_files(
  ...
  for (const result of results) {
    ...
    const provider_file = path.join(
      raw_dir,
      `${result.source}-${result.provider.name.replace('/', '-')}.json`
    );
    fs.writeFileSync(
      provider_file,
      JSON.stringify(
