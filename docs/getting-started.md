...
Minimum required variables (the defaults already work for local dev):

| Variable | Purpose |
|---|---|
| `DATABASE_URL` | SQLite file path. Default: `file:../../.desktop-dev.db`. Leave as-is for dev. |
| `JWT_SECRET` | Any random string. Dev scripts override this; fine to leave placeholder. |
| `CREDENTIAL_ENCRYPTION_KEY` | Exactly 32 characters. Dev scripts override this too. |
| `ANTHROPIC_API_KEY` | *Optional*. Enables the AI assistant. Get a key at [console.anthropic.com](https://console.anthropic.com). |

For production deployments, **you must override** `JWT_SECRET` and `CREDENTIAL_ENCRYPTION_KEY` with real random values. See `/SECURITY.md`.
...
| AI panel shows "No API key" | `ANTHROPIC_API_KEY` not set in `.env` | Add it to `.env` and restart the gateway |
