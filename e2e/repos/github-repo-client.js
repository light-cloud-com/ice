...
function getToken() {
    ...
    if (!token)
        throw new Error('ICE_TEST_GITHUB_TOKEN env var required for test repo operations');
    return token;
}
function headers() {
    return {
 ...
 */
export async function repoExists(org, repo) {
 ...
 */
export async function repoIsEmpty(org, repo) {
    const h = headers();
...
export async function pushFiles(opts) {
    ...
    try {
        ...
        if (!baseSha)
            return { success: false, error: lastError || 'Ref never became available' };
        ...
        if (!updateRefRes.ok)
            return { success: false, error: `Failed to update ref: ${updateRefRes.status}` };
        return { success: true };
    }
    catch (err) {
 ...
 * Delete a repository.
 */
export async function deleteRepo(org, repo) {
 ...
 */
export async function listTestRepos(org, prefix = 'ice-test-') {
    const h = headers();
    const names = [];
    let page = 1;
