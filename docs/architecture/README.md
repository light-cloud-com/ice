...
State for the "last-applied" graph is persisted in the runtime DB; the deploy engine diffs the desired graph against the last-applied graph to compute the plan. See [core-engine.md](core-engine.md) for the engine internals and [database.md](database.md) for where state lives.
...
See [database.md](database.md) for the schema walkthrough — and for the difference between the runtime DB (`.desktop-dev.db`) and the schema catalog (`ice-schemas.db`).
...
GCP coverage is the most complete (20 service handlers, full lifecycle). AWS and Azure are intentionally partial — they exist, they compile, many handlers work, but GCP is the only provider where we claim "production-ready" at this stage. See [core-engine.md](core-engine.md) for the plan/apply implementation, [`PROVIDER_READINESS`](../../packages/constants/src/providers.ts) for the per-provider truth-source, and [ROADMAP.md](../../ROADMAP.md) for coverage plans.
