...
interface ErrorPattern {
  ...
  category: ErrorCategory;
  gcpCode?: string;
  suggestion: string | ((msg: string, project?: string) => string);
  consoleUrl?: (msg: string, project?: string) => string | undefined;
}
...
const PATTERNS: ErrorPattern[] = [
  // API not enabled — highest priority, very common on fresh projects
  {
    ...
    consoleUrl: (_msg, project) =>
      project ? `https://console.cloud.google.com/apis/dashboard?project=${project}` : undefined,
  },

  // Authentication / token issues
  {
    test: (msg, status) =>
      status === 401 || /UNAUTHENTICATED|token.*expired|invalid.*credentials|auth.*fail/i.test(msg),
    category: 'auth',
  ...
  {
    test: (msg, status) =>
      (status === 403 || /PERMISSION_DENIED|forbidden|access denied/i.test(msg)) &&
      !/serviceusage|api.*not.*enabled/i.test(msg),
    category: 'permission',
    gcpCode: 'PERMISSION_DENIED',
    suggestion: (msg) => {
      const roleMatch = msg.match(/requires.*?(roles\/[\w.]+)/);
      if (roleMatch) return `Grant ${roleMatch[1]} to the service account.`;
      return 'Grant the required IAM role to the service account. Check the error details for the specific permission needed.';
    },
    consoleUrl: (_msg, project) =>
      project ? `https://console.cloud.google.com/iam-admin/iam?project=${project}` : undefined,
  },

  // Quota exceeded
  {
    test: (msg, status) =>
      status === 429 || /QUOTA_EXCEEDED|RESOURCE_EXHAUSTED|resourceExhausted|rate.*limit|too many requests/i.test(msg),
    category: 'quota',
    gcpCode: 'RESOURCE_EXHAUSTED',
    suggestion: 'GCP quota exceeded. Wait and retry, or request a quota increase in the GCP Console.',
    consoleUrl: (_msg, project) =>
      project ? `https://console.cloud.google.com/iam-admin/quotas?project=${project}` : undefined,
  },
  ...
  {
    test: (msg) =>
      /build.*fail|cloudbuild|dockerfile|docker.*build|COPY failed|npm.*ERR|pip.*install.*fail|build step.*fail/i.test(
        msg,
      ),
    category: 'build',
    suggestion: 'Build failed. Check the Dockerfile and source code. Review Cloud Build logs for details.',
    consoleUrl: (_msg, project) =>
      project ? `https://console.cloud.google.com/cloud-build/builds?project=${project}` : undefined,
  },
  ...
  // Timeout
  {
    ...
    suggestion:
      'Operation timed out. Some resources (Cloud SQL, GKE) take several minutes. Try increasing the timeout.',
  },

  // Network errors
  {
    test: (msg) =>
      /UNAVAILABLE|ECONNREFUSED|ENOTFOUND|ECONNRESET|getaddrinfo|network.*error|socket.*hang.*up|fetch.*fail/i.test(
        msg,
      ),
    category: 'network',
    gcpCode: 'UNAVAILABLE',
  ...
  {
    test: (msg, status) =>
      status === 400 || /INVALID_ARGUMENT|FAILED_PRECONDITION|invalid.*value|bad request|validation.*fail/i.test(msg),
    category: 'config',
    gcpCode: 'INVALID_ARGUMENT',
    suggestion: (msg) => {
      const fieldMatch = msg.match(/field\s+"?(\w+)"?/i);
      if (fieldMatch) return `Invalid value for field "${fieldMatch[1]}". Check the resource configuration.`;
 ...
 */
export function classifyDeployErrors(
  resources: Array<{ name?: string; error?: string; type?: string }>,
  phase: string,
  gcpProject?: string,
): ClassifiedError[] {
 ...
 */
export function summarizeErrors(errors: ClassifiedError[]): Record<ErrorCategory, number> {
