...
export class CanvasUI {
  ...
  async getNodes(): Promise<Array<{ nodeId: string; iceType: string }>> {
    const handles = await this.page.locator('[data-node-id]').all();
    const out: Array<{ nodeId: string; iceType: string }> = [];
    for (const h of handles) {
      const nodeId = await h.getAttribute('data-node-id');
      const iceType = await h.getAttribute('data-ice-type');
      if (nodeId && iceType) out.push({ nodeId, iceType });
    }
    return out;
  }

  async getNodeIds(): Promise<Set<string>> {
  ...
  async addBlock(iceType: string, target: { x: number; y: number } = { x: 400, y: 300 }): Promise<AddBlockResult> {
    ...
    if (!pBounds) throw new Error(`palette item not found: ${iceType}`);
    if (!cBounds) throw new Error('svg-canvas not found');
    ...
    const result = await this.page.waitForFunction(
      (existing: string[]) => {
        const els = Array.from(document.querySelectorAll('[data-node-id]')) as HTMLElement[];
        for (const el of els) {
          const id = el.getAttribute('data-node-id');
          const t = el.getAttribute('data-ice-type');
          if (id && t && !existing.includes(id)) return { id, t };
        }
        return null;
      },
      [...before],
    ...
    if (!handle) throw new Error(`addBlock: no new node appeared after drag of ${iceType}`);
    return { nodeId: handle.id, iceType: handle.t };
  }

  async deleteBlock(nodeId: string): Promise<void> {
  ...
  async selectBlock(nodeId: string): Promise<void> {
  ...
  async connectBlocks(fromNodeId: string, toNodeId: string): Promise<void> {
    ...
    if (!sBounds) throw new Error(`source port not found: ${fromNodeId}-right`);
    if (!tBounds) throw new Error(`target port not found: ${toNodeId}-left`);
  ...
  laneSlot(index: number, total: number): { x: number; y: number } {
    const startX = 220;
    const stepX = 200;
    const y = 240;
