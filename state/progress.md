...
### 2026-05-02 — Parallel-deploy + dedup follow-ups (carry-over from pdl + rf-0c)

Six items deferred during the parallel-deploy initiative and the LOC-discipline initiative are being picked up in a single batch:

- ⏸️ **pdl-11 canvas-block-default-provider** — drop handler in cards-slice should default `node.data.provider` to the active deploy provider so newly-dropped palette blocks aren't filtered as "skipped — non-GCP". Learning anchor `ux-canvas-blocks-default-no-provider-on-drag-drop` covers the failure mode.
- ⏸️ **rollupPercentage extraction** — cap-at-99 formula duplicated across `deploy-panel.tsx`, `svg-canvas.tsx`, `status-bar.tsx`. Extract once next to `deriveRollup`. Per pdl-5 critic finding #2 + #4.
- ⏸️ **Phase 2 nodesById warm-seed** — close mid-deploy tab-join sentinel-rendering window: reconstruct minimal `node_status` events from snapshot's `nodeStatuses` and dispatch `applyNodeStatusEvent` directly. Per pdl-5 critic finding #7.
- ⏸️ **DeployProgressSnapshot dead fields** — drop `progress` / `currentResource` / `currentStep` from `DeployProgressSnapshot` type and from server-side write paths (~30 lines, frontend already doesn't read them).
- ⏸️ **data.status legacy fallback** — per learning `one-status-source-deploy-status`, drop fallback in `compact-node/index.tsx:81` and sweep ~6 node-creation sites that still seed `status: 'active'` (templates, blueprints, blocks/{aws,azure,gcp}/security/waf, svg-canvas drop handlers, properties-panel post-create dispatch). Multi-package.
- ⏸️ **Cross-package dedup (3 from rf-0c)** — (a) iceType classifier set duplicated across `@ice/types` / `@ice/core` / `@ice/ui`; (b) `mapStatusToOverlay` (server) vs `mapWireStatusToOverlay` (UI) — should live in `@ice/types` next to `DeployNodeStatus`; (c) `isContainer` (`@ice/types`) vs `is_container_type` (`@ice/core`).
