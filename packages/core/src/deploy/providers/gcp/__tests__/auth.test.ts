...
function makeSdk(opts: {
  ...
  const sdk = {
    GoogleAuth: vi.fn().mockImplementation((args: any) => {
      const inst = {
        args,
        getClient: opts.authClientThrows
          ? vi.fn().mockRejectedValue(new Error('boom'))
          : vi.fn().mockResolvedValue(defaultAuthClient),
        getCredentials: vi.fn().mockResolvedValue(opts.credentials ?? { client_email: 'svc@p.iam' }),
      };
      googleAuthInstances.push(inst);
      return inst;
    }),
    OAuth2Client: vi.fn().mockImplementation((clientId: string, clientSecret: string) => {
      const inst = { ...defaultOAuthClient, clientId, clientSecret };
      oauthInstances.push(inst);
      return inst;
    }),
  };
  return { sdk, googleAuthInstances, oauthInstances };
}
...
describe('get_gcp_credentials', () => {
  ...
  it("uses GoogleAuth({scopes, projectId}) for the 'adc' method", async () => {
    const { sdk, googleAuthInstances } = makeSdk();
    sdkBag.load_sdk.mockResolvedValue(sdk);
    const cfg: GCPAuthConfig = { method: 'adc', project_id: 'p1' };

    const client = await get_gcp_credentials(cfg);

    expect(sdk.GoogleAuth).toHaveBeenCalledWith({
  ...
  it("constructs an OAuth2Client and calls setCredentials with the refresh_token for 'oauth'", async () => {
    const { sdk, oauthInstances } = makeSdk();
    sdkBag.load_sdk.mockResolvedValue(sdk);
    const cfg: GCPAuthConfig = {
      method: 'oauth',
      project_id: 'p1',
    ...
    expect(sdk.OAuth2Client).toHaveBeenCalledWith('cid', 'csec');
