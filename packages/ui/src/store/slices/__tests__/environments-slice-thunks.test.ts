...
describe('environments-slice thunks', () => {
  ...
  describe('createEnvironment', () => {
    it('fulfills with the created environment', async () => {
      ...
      const store = makeStore();
      const action = await store.dispatch(createEnvironment({ projectId: 'p-1', name: 'staging', type: 'staging' }));
  ...
  describe('renameEnvironment', () => {
    it('fulfills', async () => {
      ...
      const store = makeStore();
      const action = await store.dispatch(renameEnvironment({ envId: 'env-1', projectId: 'p-1', name: 'new-name' }));
    ...
    it('rejects on success: false', async () => {
      ...
      const store = makeStore();
      const action = await store.dispatch(renameEnvironment({ envId: 'env-1', projectId: 'p-1', name: 'x' }));
  ...
  describe('compareEnvironments', () => {
    it('fulfills with diff', async () => {
      ...
      const store = makeStore();
      const action = await store.dispatch(compareEnvironments({ sourceEnvId: 'a', targetEnvId: 'b' }));
  ...
  describe('promoteEnvironment', () => {
    it('fulfills', async () => {
      ...
      const store = makeStore();
      const action = await store.dispatch(promoteEnvironment({ sourceEnvId: 'a', targetEnvId: 'b' }));
