...
export function renderRunReport(runDir) {
    const eventsPath = join(runDir, 'events.jsonl');
    const summaryPath = join(runDir, 'summary.json');
    if (!existsSync(eventsPath))
        throw new Error(`events.jsonl not found in ${runDir}`);
...
function buildHtml(events, summary, runDir) {
    ...
    const phaseSummary = (summary?.phases ?? [])
        .map((p) => `<div class="ph"><b>${p.phase}</b> · ${p.status} · ${(p.durationMs / 1000).toFixed(1)}s${p.error ? ` · <span class="err">${escape(p.error)}</span>` : ''}</div>`)
        .join('');
    const verifySummary = (summary?.verifyResults ?? [])
        .map((v) => `<div class="vr ${v.exists ? 'ok' : 'no'}"><b>${v.resourceKind}</b>: ${v.exists ? 'exists' : 'missing'}${v.error ? ` <span class="err">${escape(v.error)}</span>` : ''}</div>`)
        .join('');
    ...
    const deployLogText = events
        ...
        .map((e, i) => `${String(i + 1).padStart(3, ' ')}  ${e.text}`)
        .join('\n');
...
function renderPayload(e, runDir) {
    switch (e.kind) {
        case 'phase_start':
...
export function renderAllRuns(rootDir) {
    const out = [];
