...
import { ALL_TEMPLATES } from '../../packages/templates/src/index';
import { test } from '../fixtures/template-deploy.fixture';
import { DeployLogCollector } from '../utils/deploy-log-collector';
    ...
    for (const template of TEMPLATES_TO_TEST) {
        test(`${template.name} [${template.category}]`, async ({ templateDeploy }) => {
            ...
            if (deployResult.result?.resources) {
                ...
                const ok = resources.filter((r) => r.success).length;
                const failed = resources.filter((r) => !r.success).length;
                progress.setResources(template.id, resources.length, ok, failed, resources.map((r) => ({
                    name: r.name,
                    type: r.type,
                    success: r.success,
                    error: r.error,
            ...
            if (deployResult.result?.resources) {
                const successResources = deployResult.result.resources.filter((r) => r.success && r.name);
            ...
            if (deployResult.success) {
                ...
                if (overlay && Object.keys(overlay).length > 0) {
                    const computeEntries = Object.entries(overlay).filter(([, data]) => typeof data?.deploy_resource_type === 'string' &&
                        /^gcp\.(storage\.bucket|run\.|cloudfunctions\.)/.test(data.deploy_resource_type));
                    const withUrl = computeEntries.filter(([, data]) => {
                        const out = (data?.deploy_outputs || {});
                        return Boolean(out.url || out.default_url);
                    });
                    if (computeEntries.length > 0) {
                        progress.addLog(`URL propagation: ${withUrl.length}/${computeEntries.length} compute blocks carry a reachable URL`);
                        // Soft-assert — log a warning but don't fail the suite on
                        // templates that don't have a compute block at all.
                        if (withUrl.length === 0) {
                            progress.addError(template.id, `Phase A regression: compute blocks have no url/default_url in overlay (${computeEntries.length} blocks checked)`);
                        }
                    }
                }
            }
