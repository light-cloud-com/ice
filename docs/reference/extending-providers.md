...
If you're new to the codebase, skim [architecture overview](../architecture/README.md) first so the terms below land.
...
- Update `PROVIDER_READINESS` in [`packages/constants/src/providers.ts`](../../packages/constants/src/providers.ts) once handlers are stable. That value drives every UI badge.
...
- [architecture overview](../architecture/README.md) - overall flow.
- [core-engine](../architecture/core-engine.md) - translate / plan / apply.
...
- [`PROVIDER_READINESS`](../../packages/constants/src/providers.ts) - the source-of-truth for what's stable vs experimental.
