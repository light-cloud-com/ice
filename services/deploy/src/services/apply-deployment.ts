...
import { getExistingNameMap, getResourceMap, seedMappingsFromHistory } from './resource-mapping.service';
...
export async function applyDeployment(
  ...
  const runBody = async () => {
    try {
      ...
      if (hasResults) {
        emitLog(cardId, `Found ${foundCount} existing resource(s) from previous deployment`);
      }
