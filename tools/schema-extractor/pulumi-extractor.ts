...
export class PulumiExtractor {
  ...
  async extract_provider(provider_id: string): Promise<ExtractionResult> {
    ...
    const [namespace, name] = provider_id.includes('/') ? provider_id.split('/') : ['pulumi', provider_id];
  ...
  private extract_validation(schema: PulumiPropertySchema): ValidationDefinition {
    ...
    if (schema.enum && schema.enum.length > 0) {
      validation.enum_values = schema.enum.map((e) => (typeof e.value === 'boolean' ? String(e.value) : e.value)) as (
        | string
        | number
      )[];
    }
