...
export function computeCompactNodeHeight(data: Record<string, unknown>, _isBlock: boolean, hasPipeline = false): number {
...
export const SvgCompactNode: React.FC<SvgCompactNodeProps> = ({
    ...
    const repository =
      (data.repository as string) || (data.github as string) || (data.repo as string) || '';
    const domain =
      (data.domain as string) || (data.subdomain as string) || (data.url as string) || '';
    ...
    // ── Icons ──
    const brandIcon: BrandIcon | null =
      getBrandIcon(runtime) || getBrandIcon(iceType) || getBrandIcon(label);
    ...
    const aggregatePipelineStatus: NodePipelineStatus | null = (() => {
      if (isSourceRepo && connectedPipelineStatuses.length > 0) {
        ...
        const active = connectedPipelineStatuses.find((p) => p.status === 'building' || p.status === 'deploying' || p.status === 'queued');
    ...
    const displayLabel = truncate(
      label || '',
      Math.max(Math.floor((W - TEXT_X - CARD_PX - 40) / 7), 8)
    );
    ...
    if (lod < 3) {
      return (
        <g data-node-id={node.id} style={{ cursor: 'move' }} onMouseEnter={onEnter} onMouseLeave={onLeave}>
          <rect x={x} y={y} width={100} height={100} rx={8} fill={lod === 1 ? 'red' : 'orange'} stroke="black" strokeWidth={2} />
          <text x={x + 50} y={y + 50} textAnchor="middle" dominantBaseline="middle" fill="white" fontSize={16} fontWeight="bold">
            L{lod}
          </text>
        </g>
      );
    ...
    if (lod <= 1) {
      ...
      const pipeColor = effectivePipelineStatus
        ? effectivePipelineStatus.status === 'success' ? '#22c55e'
          : effectivePipelineStatus.status === 'failed' ? '#ef4444'
          : '#3b82f6'
        : null;

      return (
        <g className="svg-compact-node lod-1" data-node-id={node.id} style={{ cursor: 'move' }}
          onMouseEnter={onEnter} onMouseLeave={onLeave}>
          {isSelected && <rect x={cx - S / 2 - 3 * invScale} y={cy - S / 2 - 3 * invScale} width={S + 6 * invScale} height={S + 6 * invScale} rx={12 * invScale} fill="none" stroke={cat.glow} strokeWidth={2 * invScale} opacity={0.6} />}
          <rect x={cx - S / 2} y={cy - S / 2} width={S} height={S} rx={10 * invScale} fill="var(--ice-bg-surface)" stroke={border} strokeWidth={borderW} />
          <image x={cx - iconSize / 2} y={cy - iconSize / 2 - 4 * invScale} width={iconSize} height={iconSize} href={brandIcon?.url || providerUrl} preserveAspectRatio="xMidYMid meet" />
          <circle cx={cx} cy={cy + S / 2 - 10 * invScale} r={dotR}
            fill={pipeColor || statusColor} opacity={0.9}>
            {pipeColor && effectivePipelineStatus?.status !== 'success' && effectivePipelineStatus?.status !== 'failed' && (
    ...
    if (lod <= 2) {
      ...
      return (
        <g className="svg-compact-node lod-2" data-node-id={node.id} style={{ cursor: 'move' }}
          onMouseEnter={onEnter} onMouseLeave={onLeave}>
          {isSelected && <rect x={cx - CW / 2 - 3 * invScale} y={cy - CH / 2 - 3 * invScale} width={CW + 6 * invScale} height={CH + 6 * invScale} rx={(CARD_RADIUS + 2) * invScale} fill="none" stroke={cat.glow} strokeWidth={2 * invScale} opacity={0.6} />}
          <rect x={cx - CW / 2} y={cy - CH / 2} width={CW} height={CH} rx={CARD_RADIUS * invScale} fill="var(--ice-bg-surface)" stroke={border} strokeWidth={borderW} />
          {/* Icon */}
          <image x={cx - CW / 2 + padX} y={cy - iconSz / 2} width={iconSz} height={iconSz} href={brandIcon?.url || providerUrl} preserveAspectRatio="xMidYMid meet" />
          {/* Label */}
          <text x={cx - CW / 2 + padX + iconSz + 6 * invScale} y={cy - 4 * invScale} dominantBaseline="middle" fill="var(--ice-text-primary)" fontSize={fontSize} fontWeight="600"
            fontFamily="'JetBrains Mono Variable', monospace" style={{ pointerEvents: 'none' }}>
            {truncate(label || '', 12)}
          </text>
          {/* Status dot + label */}
          <circle cx={cx - CW / 2 + padX + 4 * invScale} cy={cy + CH / 2 - 10 * invScale} r={dotR} fill={pipeColor || statusColor} opacity={0.9}>
            {pipeColor && effectivePipelineStatus?.status !== 'success' && effectivePipelineStatus?.status !== 'failed' && (
              <animate attributeName="opacity" values="1;0.4;1" dur="1.2s" repeatCount="indefinite" />
            )}
          </circle>
          {statusLabel && (
            <text x={cx - CW / 2 + padX + 12 * invScale} y={cy + CH / 2 - 9 * invScale} dominantBaseline="middle" fill="var(--ice-text-secondary)" fontSize={9 * invScale}
          ...
          {isHovered && (
            <g className="connection-ports">
              <circle className="connection-port" data-node-id={node.id} data-side="left" cx={cx - CW / 2} cy={cy} r={5 * invScale} fill={cat.glow} stroke="var(--ice-bg-base)" strokeWidth={2 * invScale} style={{ cursor: 'crosshair' }} />
              <circle className="connection-port" data-node-id={node.id} data-side="right" cx={cx + CW / 2} cy={cy} r={5 * invScale} fill={cat.glow} stroke="var(--ice-bg-base)" strokeWidth={2 * invScale} style={{ cursor: 'crosshair' }} />
            </g>
          )}
        </g>
      );
            ...
            {hasPipeline && effectivePipelineStatus && (
              ...
              >
                ...
                <text
                  ...
                  style={{ pointerEvents: 'none' }}
                >
                  {effectivePipelineStatus.status === 'success' ? 'Live' :
                   effectivePipelineStatus.status === 'failed' ? 'Failed' :
                   effectivePipelineStatus.status === 'building' ? 'Building' :
                   effectivePipelineStatus.status === 'deploying' ? 'Deploying' :
                   'Queued'}
                </text>

                {/* Progress bar (during active states) */}
                {(effectivePipelineStatus.status === 'building' || effectivePipelineStatus.status === 'deploying' || effectivePipelineStatus.status === 'queued') && (
                  <g>
                    <rect
                      x={x + CARD_PX + 60}
                ...
                {(effectivePipelineStatus.status === 'success' || effectivePipelineStatus.status === 'failed') && effectivePipelineStatus.commitSha && (
                  <text
                    x={x + W - CARD_PX}
                    y={y + H - PAD_BOTTOM - (hasStatusLine ? STATUS_LINE_H : 0) - 2}
                    textAnchor="end"
            ...
            {isSourceRepo && connectedPipelineStatuses.length > 0 && !hasPipeline && (
              <g style={{ pointerEvents: 'none' }}>
                {connectedPipelineStatuses.map((ps, i) => {
                  const dotColor =
                    ps.status === 'success' ? '#22c55e' :
                    ps.status === 'failed' ? '#ef4444' :
                    ps.status === 'building' || ps.status === 'deploying' ? '#3b82f6' :
                    ps.status === 'queued' ? '#f59e0b' :
                    '#64748b';
