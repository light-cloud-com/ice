/**
 * Tests for `firebase-hosting/result-helpers.ts` (rf-fbh-1). Pure shape
 ...
 * the rf-fbh series lands.
 */

import { describe, it, expect, beforeEach, afterEach, vi } from 'vitest';
import { TYPE, result, fail } from '../result-helpers';

describe('firebase-hosting/result-helpers', () => {
  describe('TYPE', () => {
    it('equals the canonical ICE iceType for Firebase Hosting', () => {
      expect(TYPE).toBe('gcp.firebase.hosting');
    });
  });
  ...
  describe('result()', () => {
    ...
    it('returns the success shape with name reused as resource_id', () => {
      ...
      const out = result('my-site', 'create', start);
      expect(out).toEqual({
        resource_id: 'my-site',
        name: 'my-site',
        type: TYPE,
    ...
    it('defaults overrides to an empty object when omitted', () => {
      ...
      // No extra keys beyond the base shape.
    ...
    it('shallow-merges overrides over the base shape', () => {
      const out = result('x', 'create', Date.now(), {
        provider_id: 'projects/p/sites/x',
        outputs: { primary_url: 'https://x.web.app' },
      });
      expect(out.provider_id).toBe('projects/p/sites/x');
      expect(out.outputs).toEqual({ primary_url: 'https://x.web.app' });
    ...
    it('lets overrides win the spread (e.g. action override)', () => {
      // The spread is last, so an override key replaces the base.
  ...
  describe('fail()', () => {
    ...
    it('returns the failure shape with success: false and the error string', () => {
      ...
      const out = fail('my-site', 'create', start, 'boom');
      expect(out).toEqual({
        resource_id: 'my-site',
        name: 'my-site',
        type: TYPE,
    ...
    it('reuses name as resource_id', () => {
      const out = fail('site-a', 'update', Date.now(), 'nope');
      expect(out.resource_id).toBe('site-a');
