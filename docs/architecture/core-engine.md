...
The core engine (`packages/core/`) is the provider-agnostic brain: everything about _how_ infrastructure is modelled, validated, diffed, planned, applied, and imported, with no UI and no network dependencies. Everything else in ICE is either a consumer of core or a translator into it.
...
2. **Canvas validation** (`validation/`) - higher-level UX rules: _"a static-site block with a custom-domain edge must have a github-repo configured"_. Runs on UI interactions and before plan.
...
- [README.md](README.md) for the bird's-eye view.
- [services.md](services.md) - how the `deploy` service uses this package.
- [../reference/blocks.md](../reference/blocks.md) - where the UI cards that feed the translator come from.
